...
func PresignAwsUrl(r string) string {
	u,_ := url.Parse(r)
...
func (c *RepoCache) updateMetadata() (*RepoMetadata, error) {
	...
	var err error	
        ...
        if strings.HasPrefix(repomd_url, "s3:") {
                fmt.Println("It's a S3 repository!")
		resp, err = http.Get(PresignAwsUrl(repomd_url))

        } else {
                fmt.Println("It's a HTTP repository!")
...
func (c *RepoCache) downloadDatabase(db *RepoDatabase) (string, error) {
        ...
        if update_db {
	        ...
	        if strings.HasPrefix(db_url, "s3:") {
                	resp, err = http.Get(PresignAwsUrl(db_url))
	        } else {
